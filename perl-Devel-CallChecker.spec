%{?scl:%scl_package perl-Devel-CallChecker}
...
Name:           %{?scl_prefix}perl-Devel-CallChecker
Version:        0.008
Release:        9%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(DynaLoader::Functions) >= 0.001
BuildRequires:  %{?scl_prefix}perl(Exporter)
BuildRequires:  %{?scl_prefix}perl(parent)
# Tests
BuildRequires:  %{?scl_prefix}perl(ExtUtils::CBuilder) >= 0.15
BuildRequires:  %{?scl_prefix}perl(ExtUtils::ParseXS)
BuildRequires:  %{?scl_prefix}perl(File::Spec)
BuildRequires:  %{?scl_prefix}perl(IO::File) >= 1.03
...
Requires:       %{?scl_prefix}perl(DynaLoader::Functions) >= 0.001
...
%changelog
* Mon Jan 06 2020 Jitka Plesnikova <jplesnik@redhat.com> - 0.008-9
- SCL
